...
module.exports = {
  ENOPPMTOKEN: () => ({
    ...
    details: `An [ppm token](${linkify(
      'README.md#atom-authentication'
    )}) must be created and set in the \`PULSAR_ACCESS_TOKEN\` environment variable on your CI environment.
...
Please visit your account page on [atom.io](https://atom.io/account) and to set it in the \`ATOM_ACCESS_TOKEN\` environment variable on your CI environment.`,
  ...
  ENOPULSARCLI: () => ({
    message: 'The pulsar CLI must be installed.',
    details: `The \`pulsar\` command line has to be installed in your CI environment and available in the \`PATH\` environment variable.
